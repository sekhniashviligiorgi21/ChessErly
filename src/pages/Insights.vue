<script setup>
  ...
  // --- Sparkline Data (Accuracy over last 10 games) ---
  const sparklinePoints = computed(() => {
    const recent = insights.value.slice(0, 10).reverse().map(g => g.insights?.overallAccuracy || 0)
    if (recent.length < 2) return ""
    const w = 280, h = 60
    const step = w / (recent.length - 1)
    return recent.map((val, i) => `${i * step},${h - (val / 100) * h}`).join(' ')
  })
          ...
          <div class="tab-content-area">
            ...
            <div v-if="activeTab === 'overview'" class="tab-panel">
                  ...
                  <svg class="sparkline" viewBox="0 0 280 60" preserveAspectRatio="none">
                    <defs>
                      <linearGradient id="sparkGrad" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" :stop-color="overallMeta.color" stop-opacity="0.4"/>
                        <stop offset="100%" :stop-color="overallMeta.color" stop-opacity="0"/>
                      </linearGradient>
                    </defs>
                    <polyline v-if="sparklinePoints" :points="sparklinePoints" fill="none" :stroke="overallMeta.color" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <polygon v-if="sparklinePoints" :points="`0,60 ${sparklinePoints} 280,60`" fill="url(#sparkGrad)"/>
                  </svg>
                  <span class="sparkline-label">Last 10 Games Trend</span>
                </div>
              </div>
              ...
              <div class="stat-row">
                ...
                <div class="stat-box">
                  <span class="stat-label">Total Blunders</span>
                  <span class="stat-value" style="color: #ff8a80;">{{ moveCounts.blunder + moveCounts.mistake }}</span>
                </div>
              </div>
                ...
                <div class="heatmap-grid-area">
                  <div class="heatmap-row" v-for="row in boardSquares" :key="row[0].sq[1]">
                    <div 
                      ...
                      :class="{ light: (sq.sq.charCodeAt(0) + sq.sq.charCodeAt(1)) % 2 === 0, dark: (sq.sq.charCodeAt(0) + sq.sq.charCodeAt(1)) % 2 !== 0 }"
                      :style="{ boxShadow: sq.intensity > 0 ? `inset 0 0 ${10 + sq.intensity * 30}px rgba(255, 50, 50, ${sq.intensity})` : 'none', background: sq.intensity > 0 ? `rgba(255, 50, 50, ${sq.intensity * 0.8})` : '' }"
                      :title="`${sq.sq}: ${sq.count} blunder${sq.count === 1 ? '' : 's'}`"
                    >
...
<style scoped>
  ...
  .tab-nav { display: flex; gap: 0.5rem; background: rgba(0, 0, 0, 0.2); padding: 0.4rem; border-radius: 12px; flex-shrink: 0; }
  .tab-btn { flex: 1; padding: 0.6rem 0.5rem; background: transparent; border: none; color: rgba(244, 240, 227, 0.6); font-weight: 600; font-size: 0.85rem; cursor: pointer; border-radius: 8px; transition: all 0.2s ease; }
  ...
  /* Hero Sparkline Card */
  .hero-card { display: flex; background: rgba(0,0,0,0.2); border-radius: 16px; padding: 1.5rem; gap: 1.5rem; align-items: center; border: 1px solid rgba(255,255,255,0.05); }
  .hero-left { display: flex; flex-direction: column; gap: 0.5rem; flex: 0 0 auto; }
  .hero-acc-row { display: flex; align-items: center; gap: 0.5rem; }
  .hero-icon { width: 32px; height: 32px; }
  .hero-value { font-family: "JetBrains Mono", monospace; font-size: 2.2rem; font-weight: 700; }
  .hero-right { flex: 1; display: flex; flex-direction: column; gap: 0.25rem; }
  .sparkline { width: 100%; height: 60px; }
  .sparkline-label { font-size: 0.7rem; color: rgba(244,240,227,0.5); text-transform: uppercase; letter-spacing: 0.5px; text-align: center; }
  ...
  .stat-box { background: rgba(0, 0, 0, 0.2); border: 1px solid rgba(255, 255, 255, 0.05); border-radius: 12px; padding: 1.2rem 1rem; display: flex; flex-direction: column; align-items: center; gap: 0.5rem; }
  ...
  .mini-phase { background: rgba(0,0,0,0.2); padding: 1rem; border-radius: 10px; display: flex; flex-direction: column; align-items: center; gap: 0.25rem; }
  ...
  .section-subtitle { font-family: serif; color: #f5f5dc; font-size: 1.1rem; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; margin: 0; }
  .heatmap-total { font-size: 0.8rem; color: rgba(244,240,227,0.5); font-family: "JetBrains Mono", monospace; }
  
  .heatmap-board { display: flex; gap: 5px; width: 100%; max-width: 400px; margin: 0 auto; aspect-ratio: 1/1; }
  .heatmap-rank-labels { display: flex; flex-direction: column; justify-content: space-around; width: 10px; color: rgba(244,240,227,0.4); font-size: 0.7rem; font-family: "JetBrains Mono", monospace; }
  .heatmap-grid-area { flex: 1; display: flex; flex-direction: column; }
  .heatmap-row { display: flex; flex: 1; }
  .heatmap-square { flex: 1; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; font-weight: 700; color: rgba(255,255,255,0.8); transition: background 0.2s; cursor: default; }
  .heatmap-square.light { background: var(--board-light); }
  .heatmap-square.dark { background: var(--board-dark); }
  .heatmap-square:hover { border: 1px solid #fff; z-index: 2; }
  .sq-count { text-shadow: 0 1px 2px rgba(0,0,0,0.5); }
  .heatmap-file-labels { display: flex; justify-content: space-around; height: 10px; margin-top: 2px; color: rgba(244,240,227,0.4); font-size: 0.7rem; font-family: "JetBrains Mono", monospace; }
